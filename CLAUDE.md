...
npm run build    # next build — also validates generateStaticParams() for /products/[id] and /catalog/[slug]
...
- **Next.js 16.2.6 + React 19.2.4, App Router only.** `searchParams` and `params` in page/layout/metadata functions are `Promise<...>` — you must `await` them (see [app/page.tsx](app/page.tsx) and [app/products/[id]/page.tsx](app/products/%5Bid%5D/page.tsx)). AGENTS.md warns to consult `node_modules/next/dist/docs/` before writing Next code — that warning is real, not boilerplate.
...
- Each category has its own landing page at `/catalog/<slug>` ([app/catalog/[slug]/page.tsx](app/catalog/%5Bslug%5D/page.tsx)). Its `slug`, H1, intro, comparison-table columns, SEO text and FAQ live on the `CATEGORIES` entry in `lib/products.ts`; `{minPrice}` in that copy is filled from live `/admin` prices. Link to categories via `categoryHref(key)`, never hand-built URLs.
- Legacy `/?cat=<key>` homepage URLs 308-redirect to the category page, keeping other query params (`gclid`, `utm_*`). They are still in Google's index and ads — keep the redirect.
- `Product.seoTitle` is the product page `<title>` phrased the way people search ("Интерактивная панель 65 дюймов …"); `name` stays the card label.
